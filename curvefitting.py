...
def gaussguess(x,y,nterms=4):
    ...
    x0 = np.mean(x[np.where(yguess==max(yguess))]) #Estimates central x value
    a = max(yguess) #Estimates amplitude
    ...
    sigma = fwhm[0]/(2*np.sqrt(2*np.log(2))) #Calculates sigma from FWHM

    if nterms==4:
        return [const,a,x0,sigma,fwhm[0]]
    else:
        return [a,x0,sigma]
