...
##    pdb.set_trace()
    ...
    #Set up beam
    PT.pointsource(0.,10) #multiple rays to avoid Python complaints

    #Rotate to nominal grating reference frame
    PT.transform(0,0,0,pi/2+1.5*pi/180,0,0)

    #Apply misalignment
    PT.transform(*gratAlign)

    #Trace grating
    PT.flat()
    PT.transform(0,0,0,0,0,yaw) #-1.73211678*pi/180
    PT.radgrat(4748.372,160./4748.372,order,wave)
    PT.itransform(0,0,0,0,0,yaw)

    #Reverse misalignment
    PT.itransform(*gratAlign)

    #Trace to focal plane
    PT.transform(0,4748.372,0,0,0,0)
    PT.transform(0,0,0,pi/2,0,0)
    PT.flat()

    #Return mean positions
    return mean(PT.x),mean(PT.y)

def dofTest(dof,step,order=1.,wave=2.48):
    #Get nominal beam focal plane location
    misalign = zeros(6)
    tr = lambda misalign: traceBeam(misalign,order=order,wave=wave)
    xn,yn = tr(misalign)
