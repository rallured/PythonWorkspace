...
def acquire(acqtime, disc=80, roi=[]):
  ...
  dischex = "%0.3x" % disc
  ser.write('D'+dischex) # set the discriminator level
  if sum(mcahist) > 0:
    print "mcahist not zeroed"
    mcahist[:] = 0.0
  while acqaccum < acqtime:
    ser.write('B') # start the MCAPIC
...
###rdaly::end code added for diagnostic
    ...
    mcahist = mcahist+c
    totalevents=sum(mcahist)
    #for i in range(nchan):
    #    mcahist[i] += b[2*i]+256*b[2*i+1]
    acqaccum += seconds_acq
    countrate = totalevents/acqaccum
