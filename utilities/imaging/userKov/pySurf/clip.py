...
KEYWORD PARAMETERS:
  MIN, MAX: Values in Data lower than MIN or higher than MAX are clipped. 
    MIN and MAX can be scalar or arrays of the same shape as DATA.
    If one of them is not provided the clipping is not performed in that direction.
  REFIMAGE: can be scalar or same shape as DATA.
    if provided, MIN and MAX refer to the range of the deviation of DATA from REFIMAGE.
  CLIPVALUE can be a scalar, a two element vector, a SHAPE or SHAPEx2 array with values to substitute in
    clipped points, where SHAPE is the shape of DATA (N if data is a vector, NxM if it is an array). 
    If scalar or SHAPE array, the same value is used for points smaller than MIN
  ...
  NSIGMA if it is set, data that are out of a NSIGMA interval of standard deviations from average of DATA
     are clipped (statistics are calculated on the deviation from REFIMAGE if provided,
     on the scalar average of DATA if REFIMAGE is not provided).  

OUTPUT KEYWORD:
  MASK: return a SHAPEx2 (for min and max) mask array with 1 in the position of modified values. 
  TODO: 
    - SUMMARY: return a string with the summary of the clipping
    - The procedure is almost already good for the n-dimensional case, only the part related to
      CLIPVALUE needs to be modified. Also the part related to Mask needs to be improved, it returns
      always an NxMx2 array, even in the 1D case, and it is not ready for more than 2 dimensions.
OUTPUTS:
...
PROCEDURE:
  Min, Max and clipvalue are internally converted to arrays of the same shape as Data.
...
MODIFICATION HISTORY:
  Written by: Vincenzo Cotroneo, 2014/08/11 (IDL port of old routine).
  Harvard-Smithsonian Center for Astrophysics
  60, Garden street, Cambridge, MA, USA, 02138
  vcotroneo@cfa.harvard.edu
"""

def clip (data, min=mmin,max=mmax,clipvalue=clipvalue,
    nsigma=nsigma, refimage=None,mask=mask,summary=summary,toRefImage=False)
  ...
  if toRefImage:
    and refImage == None then raise ValueError('You have set TOREFIMAGE, but REFIMAGE is not provided.')
  if refimage==None: 
    refimage=data*0 
  else:
    try:
        if len(refImage)==1:    #single element scalar
            refimage= data*0+refimage[0]
    except:
  ...
  if keyword_set(toRefImage) then begin ;the image is clipped to REFIMAGE
    ...
    if n_elements(clipvalue) eq 0 then begin 
       ;points are clipped to minimum and maximum accepted values
       minclip=min+refimage
       maxclip=max+refimage
    endif else if n_elements(clipvalue) eq 1 then begin
       ;points are clipped to the same scalar value
       minclip=replicate(clipvalue,datasize)
       maxclip=replicate(clipvalue,datasize)
    endif else if n_elements(clipvalue) eq 2 then begin
        ;the two elements define respectively values for min and max.
        minclip=replicate(clipvalue[0],datasize)
        maxclip=replicate(clipvalue[1],datasize)
    endif else begin
      ;clipvalue is one (or two for min/max) set of points with the same shape of data.
      if size(clipvalue,/n_dim) eq n_elements(datasize) then begin
        minclip=clipvalue
        maxclip=clipvalue
      endif else begin      
        if size(clipvalue,/n_dim) ne n_elements(datasize)+1 then message,'wrong number of dimensions for CLIPVALUE array'  
        ;for now implement manually only the 1 and two dimensional cases.
        ;a generic case can be implemented with reform and rebin
        if size(clipvalue,/dimensions) eq 2 then begin 
          minclip=clipvalue[*,0]
          maxclip=clipvalue[*,1]
        endif else begin
          if size(clipvalue,/dimensions) eq 3 then begin
            minclip=clipvalue[*,*,0]
            maxclip=clipvalue[*,*,1]
...
pro run_tests_1d
     npx=100
     zr=[-0.4,1.0]
     ;result=dialog_message('execute 1-D test 1?',/information,/cancel)
     set_plot_default
     ;if result eq 'OK' then begin
     setstandarddisplay
     x=findgen(npx)/npx*2*!PI
     y=sin(x)
     plot,x,y,yrange=[-1,4]
     oplot,x,clip(y,nsigma=1),color=5
     oplot,x,clip(y,min=-0.6,max=0.5),color=8
     oplot,x,clip(y,min=-0.6,max=0.5,refimage=x,/toref),color=12
     oplot,x,clip(y,refimage=x-1,/toref,nsigma=1),color=18
     legend,["sin(x)","clip(y,nsigma=1)","clip(y,min=-0.6,max=0.5)",$
          "clip(y,min=-0.6,max=0.5,refimage=x,/toref)",$
          "clip(y,refimage=x-1,/toref,nsigma=1)"],color=[255,5,8,12,18]
end
