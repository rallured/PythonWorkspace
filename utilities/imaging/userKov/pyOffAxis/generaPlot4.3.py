...
def generaPlot(rootdir,resdir,fileLista="lista.txt",enTarget=30.,requirements=[],
    ...
    for config in a:
        try: config,onAxisFolder=config.split()
        except: onAxisFolder=None
        print count+1, config
        Weight,ang,FOV,onAx=extractInfo.offAxisInfo(config ,enTarget,onAxisFolder)[0:4]
...
def calcolaFOV_Grasp(rootdir,resdir,fileLista,enTarget,requirements,spiderFactor,Wfactor,areaCopyFlag,
    ...
    spiderFactor non incide sui dati numerici, ma solo sui plot. 
    ...
    for config in a:
        ...
        plt.write(label)
        ss="plot '" + config + "_grasp.txt' u 1:2 w l title '"+str(FOVfraction)+" on axis Aeff"
        if (FOVfraction == FOVfraction2):
            ss=ss+"'\n"
        else:
            ss=ss+"',\\\n'" + config + "_grasp.txt' u 1:3 w l title '"+str(FOVfraction2)+" on axis Aeff'\n"
        plt.write(ss)
...
def calc(rootdir,resdir,enTarget,fileLista,spiderFactor,Wfactor,areaCopyFlag,cutoffRatio=0.05,
    ...
    plotRoutines.cmpPlot(resdir,overwrite=1,groupFile="lista.txt",xrange="[*:*]",yrange="[*:*]",pltFilePrefix="Grasppng_",
        outPrefix="Grasp_",term="ps",
        using="u 1:4",ylabel="Grasp (Aeff x area FOV)",title="GRASP")
...
if __name__=="__main__":
    ...
    calc(rootdir=r"C:\Documents and Settings\Vincenzo\Desktop\wfmxt",
        resdir="FOV_energy",
        enTarget=4.,
        fileLista='lista.txt',
        spiderFactor=0.9,
        ...
        areaCopyFlag=1,
        taroccoFactor=1.00, #fattore di moltiplicazione per il FOV per taroccare i dati
        requirements={1.0:8000/3.,4.0:2800/3.}, ###{0.8:100,2:1000,8:600,20:450,40:450,70:100}
        cutoffRatio=0.05,FOVfraction=0.5,FOVfraction2=0.2)
